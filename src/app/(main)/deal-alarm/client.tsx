...
export function DealAlarmClient({ initialAlerts, categories }: { initialAlerts: any[], categories: any[] }) {
  ...
  const [minTemperature, setMinTemperature] = useState(100) // Default to HOT deals

  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault()
    if (!keyword.trim() && !categoryId) {
      toast.error('Vui lòng nhập từ khoá hoặc chọn danh mục')
      return
    }
    ...
    startTransition(async () => {
      try {
        ...
        // Include the category name manually for the UI
        const cat = categories.find(c => c.id === categoryId)
        setAlerts([{ ...newAlert, category: cat ? { nameVi: cat.nameVi } : null }, ...alerts])
        
        setKeyword('')
        setCategoryId('')
        setMinTemperature(100)
        toast.success('Tạo báo thức thành công!')
      } catch (err: any) {
  ...
  const handleToggle = (id: string, active: boolean) => {
    startTransition(async () => {
      try {
        ...
        toast.error('Không thể bật/tắt báo thức')
      }
    })
  }

  const handleDelete = (id: string) => {
    if (!confirm('Bạn có chắc muốn xoá báo thức này?')) return
    startTransition(async () => {
      try {
  ...
  return (
    <div className="space-y-8">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Từ khoá (Ví dụ: iphone, macbook, shopee)</label>
              <Input 
                value={keyword}
                onChange={e => setKeyword(e.target.value)}
                placeholder="Để trống nếu chỉ muốn chọn danh mục"
                className="bg-card"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Chỉ nhận thông báo khi deal đạt độ HOT này (0 = Mọi deal mới, 100 = Deal HOT).
            </p>
          </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold">Báo thức của bạn</h2>
        
        {alerts.length === 0 ? (
          <div className="text-center py-12 card border-dashed">
            <AlertTriangle className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
            <p className="text-muted-foreground">Bạn chưa cài đặt báo thức nào.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {alerts.map(alert => (
              <div key={alert.id} className={`card p-4 flex justify-between items-start transition-opacity ${!alert.active ? 'opacity-60 grayscale' : ''}`}>
                <div className="space-y-1">
                  <div className="font-bold text-lg flex items-center gap-2">
                    {alert.keyword ? `Từ khoá: "${alert.keyword}"` : 'Mọi từ khoá'}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Danh mục: <span className="font-medium text-foreground">{alert.category?.nameVi || 'Tất cả'}</span>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Nhiệt độ tối thiểu: <span className="font-medium text-hot">{alert.minTemperature}°</span>
                  </div>
                </div>
